...
func bone_tornado():
	...
	for i in 50:
		...
		if i % 6 == 2:
			...
			bone(box.center+off,1,0,Vector2(60,60),-12,0,true) # Right Orange Bone
		siner += PI/10.0
...
func left_or_right():
	...
	match dir:
		"left":
			bone_stab(box.corner_left,0,90,12,50,wait,60)
		"up":
			bone_stab(box.corner_right,0,180,25,50,wait,60,5)
		"down":
			bone_stab(box.corner_b_left,0,0,25,50,wait,5)
		"right":
			bone_stab(box.corner_bottom,0,-90,12,50,wait,60)
...
func blaster_massacre():
	...
	for i in 30:
		...
		fs_gaster_blaster(start_pos,_pos,_ang,0.75,0)
...
func circle_and_blaster():
	...
	box.yscale = 0
	await get_tree().create_timer(0.05).timeout
	player.position = box.center + Vector2(0,-20)
	player.lock = false
