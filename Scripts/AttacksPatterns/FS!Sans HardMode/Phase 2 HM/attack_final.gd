		...
		for i in 4:
			if x != 0:
				if i % 2 == 0:
					...
					_sBone(box.corner_right+off,20,-4,true)
				else:
					var off = Vector2(0,10)
					_sBone(box.corner_bottom+off,-20,-4)
			else:
				if i % 2 == 0 and i != 0:
					var off = Vector2(30,-20)
					_sBone(box.corner_right+off,20,-4,true)
...
func left_or_right():
	...
	match dir:
		"left":
			bone_stab(box.corner_left,0,90,12,60,wait,60)
		"up":
			bone_stab(box.corner_right,0,180,15,50,wait,60,5)
		"down":
			bone_stab(box.corner_b_left,0,0,13,50,wait,5)
		"right":
...
func _sBone(_pos:Vector2,_angle:float,xspd=3,_inv:bool = false):
	for i in 2:
		sine_bone(_pos,_angle,30,70,_inv).xspeed = xspd
