	...
	while _loop < 3:
		match _loop:
			0:
				...
				await get_tree().create_timer(0.5).timeout
				bone_cross(Vector2(700,300),-2.5,0,50,-4)
				...
				player.set_mode(PlayerSoul.MODE.BLUE,"up",true)
				var bb = styled_bone_stab(box.corner_right-Vector2(0,40),0,180,32,30,5,550)
			...
			1:
				for i in 10:
					for j in 2:
						...
						rnd.randomize()
						var _pos = Vector2(rnd.randi_range(80,560),450+rnd.randf_range(-30,30))
						var bo = bone(_pos,1,0,Vector2(10,10),0,0,true)
						create_tween().tween_property(bo,"angle",400,2.5).set_trans(Tween.TRANS_SINE)
						boomerang(bo,"position:y",_pos.y,_pos.y-180,3)
						await get_tree().create_timer(0.25).timeout
					
					plateform(Vector2(0,350),0,0,40,Vector2(3,0))
					plateform(Vector2(640,310),0,0,40,Vector2(-3,0))
					bone(Vector2(50,400),0,0,Vector2(80,0),3,0,true)
					bone(Vector2(580,400),2,0,Vector2(80,0),-3,0,true)
					await get_tree().create_timer(0.25).timeout
...
func do(bb):
	for i in bb.bones.get_child_count():
		if i <5 or i > 27:
			...
			twed.set_parallel()
			twed.tween_property(b,"offset_top",45+20,1).as_relative().set_trans(Tween.TRANS_SINE)
